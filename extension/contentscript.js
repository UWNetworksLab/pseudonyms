...
//Checks to see if the pid given by the cookie matches what we see in this context(tab)
function checkPIdMatching(PId){
/*
...
chrome.extension.sendMessage({type : 'cookieBootstrap', url : document.URL}, 
    function(cString) {        
        _cookieDiv.innerText = cString;
    });
    
//Listen for any update messages from the background script. These message will be sent if any cookie in the store is updated and the tab matches.
chrome.extension.onMessage.addListener(
    function(request, sender, sendResponse){            
        if(checkPIdMatching(request.PId)){            
